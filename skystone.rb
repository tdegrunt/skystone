...
class StorageCartSystem
  def initialize(plugin, player)
    ...
    plugin.event(:vehicle_move) do |event|
      to = event.get_to
      from = event.get_from
      if to.get_x.to_i != from.get_x.to_i || to.get_z.to_i != from.get_z.to_i
...
class SkyStonePlugin
  ...
  def on_enable
    server.broadcast_message "Loaded SkyStone plugin"
    cart_routes = CartRoutes.new(self)

    public_command('ssp', 'SkyStone plugin', '/ssp ...') do |me, *args|
      begin
        case args.first
        ...
        when 'eval'
          args.shift
          me.msg eval(args.join(" "))

        when 'scs'
          storage_cart_system = StorageCartSystem.new(self,me)
